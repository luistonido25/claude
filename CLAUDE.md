...
- `challenges.html` + `challenges.js` — Portfolio Lab: `window.PORTFOLIO_CHALLENGES` is an array of 30 client-brief objects (`id` c01–c30, `stack` "make"|"ghl"|"ghl+n8n", `brief[]`, `guide[]` of `{step, tool, detail}` with `**bold**` node names rendered by `md()` in app.js, `dataModel[]`, `edgeCases[]`, `acceptance[]`, `portfolio[]`, `stretch[]`). c01–c18 have architecture-level guides; c19–c30 are full-combo systems (CRM + funnels + automation) whose guides are click-by-click solutions with phase-prefixed steps ("A1 —", "B1 —", "C1 —"). Cards are rendered once by `renderChallengesPage()`; filters only toggle visibility. "Built" checkboxes use `data-task="ch-NN"` and ride the normal tasks sync.
